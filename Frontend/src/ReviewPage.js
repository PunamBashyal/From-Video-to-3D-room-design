// src/components/ReviewVideo.js
// Receives video via navigate(state) from RoomSetup
// Shows preview with Back and Continue buttons
...
export default function ReviewVideo() {
  ...
  // video URL and the original File object passed from RoomSetup
  const videoURL  = location.state?.videoURL;
  const videoFile = location.state?.videoFile;

  // If someone lands here directly without a video, send them back
  if (!videoURL) {
    return (
      <div style={{
        ...
        <button
          onClick={() => navigate("/")}
          style={{
            background: "#3b2f8f", color: "#fff", border: "none",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          {/* Continue — go to room selection, carry file forward */}
          <button
            onClick={() =>
